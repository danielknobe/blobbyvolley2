...
const float BLOBBY_SPEED = 360; // Verified
const float BLOBBY_JUMP_VELOCITY = 1171.875;
const float BLOBBY_JUMP_GRAVITATION = 2000.75;

// const float BLOBBY_JUMP_VELOCITY = 850;
...
void PhysicWorld::step()
{
	...
	if (mLeftBlobPosition.y >= GROUND_PLANE_HEIGHT)
		...
		{
			mLeftBlobJumping = true;
			mLeftBlobVelocity.y = -BLOBBY_JUMP_VELOCITY;
		}
	
	if (mLeftBlobJumping)
		if (!mLeftPlayerInput.up)
			mLeftBlobJumping = false;
			
	if (mRightBlobPosition.y >= GROUND_PLANE_HEIGHT)
		if (mRightPlayerInput.up)
		{
			mRightBlobJumping = true;
			mRightBlobVelocity.y = -BLOBBY_JUMP_VELOCITY;
		}
	
	if (mRightBlobJumping)	
		if (!mRightPlayerInput.up)
			mRightBlobJumping = false;
	...
	if(ballHitLeftPlayer())
	{
		...
		mBallPosition -= mBallVelocity.scale(10);
		mBallVelocity = mBallVelocity.scale(10);
	}

	if(ballHitRightPlayer())
	{
	...
	&& mBallPosition.x+BALL_RADIUS<=NET_POSITION_X+NET_RADIUS
    && mBallVelocity.x<0
	...
	).length() < NET_RADIUS + BALL_RADIUS)
	{
		...
		mBallVelocity = mBallVelocity.normalise();
		mBallVelocity = mBallVelocity.scale(temp);
	}
	...
	if (mLeftBlobJumping)
		mLeftBlobVelocity.y += BLOBBY_JUMP_GRAVITATION * TIMESTEP;
	else
		mLeftBlobVelocity.y += 2 * BLOBBY_JUMP_GRAVITATION * TIMESTEP;

	if (mRightBlobJumping)
		mRightBlobVelocity.y += BLOBBY_JUMP_GRAVITATION * TIMESTEP;
	else
		mRightBlobVelocity.y += 2 * BLOBBY_JUMP_GRAVITATION * TIMESTEP;
